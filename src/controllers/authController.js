...
// ─── Verify Email (no OTP, no validator — just check duplicate) ───────────────
exports.verifyEmail = async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ message: "Email verified successfully", email: user.email });
  } catch (error) {
...
exports.completeSignup = async (req, res) => {
  try {
    ...
    const user = await User.findOne({ email, isEmailVerified: true });
    if (!user) return res.status(400).json({ message: "Email not verified" });
    if (user.isProfileComplete) return res.status(400).json({ message: "Profile already completed" });
    ...
    res.status(200).json({
      message: "Signup completed successfully",
      user: { id: user._id, email: user.email, name: user.name, profilePic: user.profilePic },
    });
  } catch (error) {
    console.log("COMPLETE SIGNUP ERROR:", error);
    res.status(500).json({ message: "Signup failed" });
...
exports.login = async (req, res) => {
  try {
    ...
    if (!user.isProfileComplete) return res.status(400).json({ message: "Please complete your signup first" });
    ...
    res.status(200).json({
      message: "Login successful",
      token,
      user: { id: user._id, email: user.email, name: user.name, gender: user.gender, profilePic: user.profilePic },
    });
  } catch (error) {
    console.log("LOGIN ERROR:", error);
    res.status(500).json({ message: "Login failed" });
...
exports.getUserProfile = async (req, res) => {
  try {
    ...
    res.status(200).json({
      user: { id: user._id, email: user.email, name: user.name, gender: user.gender, profilePic: user.profilePic, createdAt: user.createdAt },
    });
  } catch (error) {
    console.log("GET USER PROFILE ERROR:", error);
    res.status(500).json({ message: "Failed to fetch profile" });
